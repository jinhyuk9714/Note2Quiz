  ...
  if (isStatsLoading || isDueLoading) {
    return (
      <div className="mx-auto max-w-4xl pb-12">
        <div className="flex h-64 items-center justify-center rounded-[2rem] border-2 border-dashed border-border-default bg-surface-card/50 backdrop-blur-sm">
    ...
    <div className="mx-auto max-w-4xl space-y-10 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div
          className={cn(
            ...
            (schedule?.overdue_count ?? 0) > 0
              ? "border-red-100 dark:border-red-700 bg-red-50 dark:bg-red-900/20"
        ...
        >
          ...
          <p className="mt-1 text-xs font-bold text-text-tertiary">기한 지남</p>
        </div>
        ...
        <div
          className={cn(
            ...
            (schedule?.today_count ?? 0) > 0
              ? "border-amber-100 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20"
              : "border-border-default bg-surface-card",
          )}
        >
          <CalendarCheck
          ...
          <p className="mt-1 text-xs font-bold text-text-tertiary">오늘 복습</p>
        </div>

        <div className="flex flex-col items-center justify-center rounded-3xl border border-border-default bg-surface-card p-6 shadow-sm transition-all">
          <Clock className="h-6 w-6 text-text-tertiary" />
          <p className="mt-2 text-3xl font-black text-text-tertiary">
            {upcomingTotal}
          </p>
          <p className="mt-1 text-xs font-bold text-text-tertiary">다가오는 복습</p>
        </div>
      </div>
      ...
      {weakConcepts.length > 0 && (
        <div className="space-y-3">
          <p className="px-1 text-xs font-black uppercase tracking-[0.1em] text-text-tertiary">
            개념별 필터
          </p>
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    selectedTag === c.tag
                      ...
                      : "bg-surface-card text-indigo-600 ring-1 ring-inset ring-border-default hover:text-indigo-700",
                  )}
      ...
      {dueTotal > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-[2.5rem] border-2 border-dashed border-border-default bg-surface-card/50 p-16 text-center backdrop-blur-sm">
          <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 dark:text-emerald-400">
            <CheckCircle2 className="h-8 w-8" />
          </div>
          <h3 className="text-lg font-bold text-text-primary">
            모든 복습을 마쳤습니다!
          </h3>
          <p className="mt-2 max-w-xs text-sm font-medium leading-relaxed text-text-tertiary">
      ...
      {schedule && schedule.upcoming.length > 0 && (
        <div className="overflow-hidden rounded-3xl border border-border-default bg-surface-card p-8 shadow-sm">
          <div className="mb-4 flex items-center gap-2 px-1">
            <Clock className="h-4 w-4 text-text-tertiary" />
            <p className="text-[11px] font-black uppercase tracking-[0.1em] text-text-tertiary">
              주간 계획
            </p>
          </div>
          <ul className="space-y-2">
            {schedule.upcoming.map((day) => (
              ...
              >
                ...
                <span className="rounded-lg bg-surface-card px-2.5 py-1 text-xs font-bold text-text-secondary shadow-sm ring-1 ring-border-default transition-all group-hover:text-indigo-600 group-hover:ring-indigo-200">
                  {day.count}개
                </span>
              </li>
            ))}
